...
import { createStorailMutationClient } from "../src/index.js";

const env = loadEnvFiles([new URL("../.env", import.meta.url)]);
...
const account = privateKeyToAccount(keccak256(stringToHex("storail:sdk:e2e:user:v1")));
...
const path = `/${account.address}/apps/storail-sdk-e2e/runs/${runId}/record` as const;
const contentHash = keccak256(stringToHex(`storail-sdk-e2e:${runId}:content`));
...
const operation = await client.publish(
  {
    ...
    pointer: `synthetic-sdk-e2e-${runId}`,
    contentHash,
    metadata: JSON.stringify({ module: "sdk", runId }),
  },
  {
    gasLimit: 650_000n,
    onStatus: (next) => {
